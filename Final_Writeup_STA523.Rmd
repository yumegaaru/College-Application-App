...
In some of the older posts, the GRE scores were in older scale:200-800. In order to analyze them together with new GRE scores, we converted the old GRE scores in the dataset to match the new GRE scoring scale used today based on the 'convert table' from ETS.  
...
To clean the GRE score, we set up conditions to filter out out-of-range scores in `gradcafe.Rdata`.

To clean the institution names, we created a csv file including all the keywords for each institution to detect and rename the same schools under a standardized name. The `School Names - Sheet1.csv` file is a google spreadsheet that contains most common schools in the gradcafe dataset, and all the key words we use to recognize those schools' name. The schools in our csv files cover the top 75 statistics programs declared by US_news, as well as some other popular school that we found in the gradcafe dataset.

We then scraped the US_news top Statistics programs, and get the college name, location, rank and average scores. The final results are stored as `us_news.R`, which helped us clean the institution names.

Then we did the keyword detection to match institution names in `gradcafe.Rdata` to the standardizes institution names in the csv file and `us_news.R`. Since many of the posts contains spelling error, we also tried to capture some of the most commonly mispelled school names, such as word 'university'.
...
By pasting college names to the base url of wikipedia webpage, we scraped the introduction paragraph from the wikipage of each college. Then we also extract the picture, logo, and seals of that college from the same webpage. We used the selector gadget to identify nodes for each part, and then clean them with regular expressions.  
We ended up with a list of introductions, pictures, logos and seals for each school. The results were stored as `wiki.Rdata`. 
...
Next we created a ShinyApp to configure the data from gradcafe.com into an interactive interface. We added an introduction portion in the app in order to have a brief summary about the university. The app was built with `shinydashboard` package.
...
The user interface includes following features:
  ...
  iii. a check box: 'Provide Your Scores?': If checked, there will be four slidebar for users to input their own GPA, and GRE Verbal/Math/Writing scores.
  iv. a check box: 'specify student type?': If checked, users can see the results of only American student, international students, or international students with US degree. 
...
i. Boxplots, lineplots and tables for GPA, GRE V/M/Q scores
ii. Enrollment rates, which includes acceptance, rejection, and waitlisted
iii. Statistics for GRE subject test


Discussion:

The app showed some interesting findings. We found that different school focus on different scores. Havard, for example , seems to prefer students with high GRE verbal and writing. Duke, on the contrary, focus more on quantitative scores.
Another finding will be, American students usually have higher acceptance rate than international students. However, this may due to the fact that there are much more international students apply for Stats program than American students.

This app is imperfect. Even though we have over 4000 data entries, there are still limited data for each college. Therefore, it's hard to do statistical analysis on this data, or a draw some general conclusions. As more and more new posts appear on gradcafe, our app will be more and more helpful.  
