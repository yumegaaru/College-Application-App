...
## Step 2) Clean the GRE scores 

We converted the old GRE scores in the dataset to match the new GRE scoring scale used today. 
...
## Step 3) Clean the institution names 

Next, we cleaned up the institution names to standardize them. (For example, the Georgia Institute of Technology could have been referred to as GA Tech.) We created a csv file including all the keywords for each institution to detect and rename the same schools under a standardized name. All institutions in our dataset are in the top 75 statistics graduate programs as declared by US News.  
...
To include descriptions, logos and seals of the different colleges, we scraped Wikipedia using the selector gadget to identify elements and regular expressions to clean them. 
...
## Step 5) Generate the ShinyApp using the finalized dataframe from Step 3. 

Next we created a ShinyApp to configure the data from gradcafe.com into an interactive interface. We added an introduction portion in the app in order to have a brief summary about the university. 
